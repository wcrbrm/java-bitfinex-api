...
public class BitfinexRequest {
    ...
    public BitfinexRequest(String requestUrl)  throws BitfinexApiException {

        this.requestUrl = requestUrl;
        try {
            url = new URL(requestUrl);
        } catch (MalformedURLException e) {
            throw new BitfinexApiException("Mailformed URL " + e.getMessage());
        }
     ...
     */
    public BitfinexRequest connect() throws BitfinexApiException {
        ...
        try {
            ...
            throw new BitfinexApiException("SSL Error " + e.getMessage() );
        } catch (KeyManagementException e) {
            throw new BitfinexApiException("Key Management Error " + e.getMessage() );
        }

        try {
            conn = (HttpsURLConnection)url.openConnection();
        } catch (IOException e) {
            throw new BitfinexApiException("HTTPS Connection error " + e.getMessage());
        }

        try {
            conn.setRequestMethod(method);
        } catch (ProtocolException e) {
            throw new BitfinexApiException("HTTP method error " + e.getMessage());
        }
     ...
     * @throws BitfinexApiException in case of any error
     */
    public BitfinexRequest read() throws BitfinexApiException {
        if (conn == null) {
            connect();
        }
        try {
            ...
            if (conn.getResponseCode() >= HttpURLConnection.HTTP_BAD_REQUEST) {
                ...
                if (obj.has("code") && obj.has("msg")) {
                    throw new BitfinexApiException("ERROR: " +
                            obj.get("code").getAsString() + ", " + obj.get("msg").getAsString() );
                }
            }
        } catch (IOException e) {
            throw new BitfinexApiException("Error in reading response " + e.getMessage());
        }
        return this;
    }

    public BitfinexRequest payload(JsonObject payload) {
